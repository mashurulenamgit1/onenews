...
class PostWorldController extends Controller
{
    ...
    public function create()
    {
    	...
    	return view('admin.post.world.create',compact('category'));
    }
    ...
    public function store(Request $request)
    {
    	...
    	if ($post_image) {
                ...
                DB::table('post__worlds')->insert($data);
                $notification=array(
    ...
    public function index()
    {
    	$post=DB::table('post__worlds')->join('postcategories','post__worlds.category_id','postcategories.id')
    	      ->select('post__worlds.*','postcategories.category_name')->get();
    	return view('admin.post.world.index',compact('post'));      
    }

    public function destroy($id)
    {
    	$post=DB::table('post__worlds')->where('id',$id)->first();
    	$image=$post->post_image;
    	unlink($image);
    	DB::table('post__worlds')->where('id',$id)->delete();
    	$notification=array(
    ...
    {
    	$post=DB::table('post__worlds')->where('id',$id)->first();
    	return view('admin.post.world.edit',compact('post'));
    }
    ...
    public function update(Request $request,$id)
    {
    	...
    	$post_image=$request->file('post_image');
    	 if ($post_image) {
                ...
                Image::make($post_image)->resize(400,240)->save('public/media/post/'.$image_one_name);
                $data['post_image']='public/media/post/'.$image_one_name;
                DB::table('post__worlds')->where('id',$id)->update($data);
                $notification=array(
    ...
    {
         DB::table('post__worlds')->where('id',$id)->update(['status'=> 0]);
         $notification=array(
    ...
    {
         DB::table('post__worlds')->where('id',$id)->update(['status'=> 1]);
         $notification=array(
