...
class PostJobController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin.post.job.create',compact('category'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($post_image) {
                ...
                DB::table('post__jobs')->insert($data);
                $notification=array(
    ...
    public function index()
    {
        $post=DB::table('post__jobs')->join('postcategories','post__jobs.category_id','postcategories.id')
              ->select('post__jobs.*','postcategories.category_name')->get();
        return view('admin.post.job.index',compact('post'));      
    }

    public function destroy($id)
    {
        $post=DB::table('post__jobs')->where('id',$id)->first();
        $image=$post->post_image;
        unlink($image);
        DB::table('post__jobs')->where('id',$id)->delete();
        $notification=array(
    ...
    {
        $post=DB::table('post__jobs')->where('id',$id)->first();
        return view('admin.post.job.edit',compact('post'));
    }
    ...
    public function update(Request $request,$id)
    {
        ...
        $post_image=$request->file('post_image');
         if ($post_image) {
                ...
                Image::make($post_image)->resize(400,240)->save('public/media/post/'.$image_one_name);
                $data['post_image']='public/media/post/'.$image_one_name;
                DB::table('post__jobs')->where('id',$id)->update($data);
                $notification=array(
    ...
    {
         DB::table('post__jobs')->where('id',$id)->update(['status'=> 0]);
         $notification=array(
    ...
    {
         DB::table('post__jobs')->where('id',$id)->update(['status'=> 1]);
         $notification=array(
